...
return {
  ...
  keys = {
    ...
    -- Fill current cell with the contents of the cell directly above. A
    -- Lua function rather than a raw keystroke string: yanking with yi|
    -- moves the cursor to the start of whatever it just yanked, so a
    -- naive "{|"zyi|}| round trip lands back on the wrong cell (}|'s cell
    ...
    -- restoring it with nvim_win_set_cursor sidesteps relying on any
    -- motion to "return" correctly.
    --
    ...
    --
    -- Register z (not ""/"+) so this doesn't clobber the clipboard the
    -- way <Leader>tc's yank intentionally does. The change operator's own
    -- delete also writes to the unnamed register regardless of z, which
    -- with 'clipboard=unnamedplus' (init.lua) leaks to the system
    -- clipboard too — caught via headless testing. "_ci| (black hole
    -- register) discards the old cell content without writing it
    -- anywhere, unlike <Leader>tR's plain ci|, where leaving the replaced
    -- text in the unnamed register is normal, expected c-operator
    -- behavior, not something to guard against. ci|<C-r>z within one
    -- :normal call (not two) so the register paste happens while still in
    -- the Insert mode ci| entered — :normal implicitly exits Insert mode
    -- at the end of its own execution, which is exactly the desired final
    -- state here (unlike <Leader>tR, which deliberately stays in Insert
    -- mode and so is a plain keymap rather than a :normal call).
    {
      "<Leader>t'",
      function()
      ...
      desc = "Fill cell from cell above",
    },
  },
}
